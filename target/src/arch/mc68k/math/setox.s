/* setox.s - Motorola 68040 FP exponential routines (EXC) */
...
DESCRIPTION
	...
	The entry point __x_setox computes the exponential of a value.
	__x_setoxd does the same except the input value is a denormalized
	number.	__x_setoxm1 computes exp(X)-1, and __x_setoxm1d computes
	...
	The program __x_setox takes approximately 210/190 cycles for input
	...
	The program __x_setoxm1 takes approximately ???/??? cycles for input
	...
	Notes:	The usual case should take the branches 1.1 -> 1.3 -> 2.
		 ...
		 see the notes on Step 1 of __x_setox.
	...
	Notes:	See the notes on Step 2 of __x_setox.

	Step 3.	Calculate X - N*log2/64.
		3.1	R := X + N*L1, where L1 := single-precision(-log2/64).
		3.2	R := R + N*L2, L2 := extended-precision(-log2/64 - L1).
	Notes:	Applying the analysis of Step 3 of __x_setox in this case
	...
	Step 10.	Calculate exp(X)-1 for |X| >= 70 log 2.
		10.1 If X >= 70log2 , exp(X) - 1 = exp(X) for all practical
		 purposes. Therefore, go to Step 1 of __x_setox.
...
__x_setox	IDNT	2,1 Motorola 040 Floating Point Software Package
...
EXPMAIN:
|--Step 2.
|--This is the __x_normal branch:	2^(-65) <= |X| < 16380 log2.
...
|--Steps 8.2 -- 8.6
	...
	fmovex		fp0,fp1
	.long 0xf23c4423,0x42b8aa3b	/*  fmuls  &0x42B8AA3B,fp0 */
	fmovemx	 fp2/fp3,a7@-		|...save fp2
	movel		#1,a6@(ADJFLAG)
	fmovel		fp0,d0		|...N = int( X * 64/log2 )
	lea		EXPTBL,a1
...
|--a0 points to 2^(J/64), D0 and a1 both contain M
	fmovex		fp0,fp2
	.long 0xf23c4423,0xbc317218	/*  fmuls  &0xBC317218,fp0 */
	fmulx		L2,fp2		|...N * L2, L1+L2 = -log2/64
	faddx		fp1,fp0	 	|...X + N*L1
	faddx		fp2,fp0	 	|...fp0 is R, reduced arg.
...
|--[R*S*(A2+S*(A4+S*A6))] + [R+S*(A1+S*(A3+S*A5))]
	...
	.long 0xf23c4500,0x3950097b	/*  fmoves  &0x3950097B,fp2 */
|	MOVE.w		#0,a1@(2)	|...load 2^(J/64) in cache

	fmulx		fp1,fp2		|...fp2 IS S*A6
	fmovex		fp1,fp3
	.long 0xf23c45a3,0x3ab60b6a	/*  fmuls  &0x3AB60B6A,fp3 */
	...
	.long 0xf23c45a2,0x3f000000	/*  fadds  &0x3F000000,fp3 */
...
|--Step 9	exp(X)-1 by a simple polynomial
	...
	fmovemx	fp2/fp3,a7@-		|...save fp2
	.long 0xf23c4480,0x2f30caa8	/*  fmoves  &0x2F30CAA8,fp1 */
	fmulx		fp0,fp1		|...fp1 is S*B12
	.long 0xf23c4500,0x310f8290	/*  fmoves  &0x310F8290,fp2 */
	.long 0xf23c44a2,0x32d73220	/*  fadds  &0x32D73220,fp1 */

	fmulx		fp0,fp2		|...fp2 is S*B11
	fmulx		fp0,fp1		|...fp1 is S*(B10 + ...

	.long 0xf23c4522,0x3493f281	/*  fadds  &0x3493F281,fp2 */
...
|--Step 10	|X| > 70 log2
	...
	jgt 		EXPC1
|--Step 10.2
	.long 0xf23c4400,0xbf800000	/*  fmoves  &0xBF800000,fp0 */
	fmovel		d1,fpcr
