/* slogn.s - Motorola 68040 FP natural logarithm routines (EXC) */
...
DESCRIPTION
	...
	__x_slogn computes the natural logarithm of an
	input value. __x_slognd does the same except the input value is a
	denormalized number. __x_slognp1 computes log(1+X), and __x_slognp1d
	...
	Speed: The program __x_slogn takes approximately 190 cycles for input
		argument X such that |X-1| >= 1/16, which is the the usual
		situation. For those arguments, __x_slognp1 takes approximately
	...
	Step 4. Reconstruct log(X) = log( 2**k * Y ) =
		k*log(2) + log(F) + log(1+u)
...
__x_slogn	IDNT	2,1 Motorola 040 Floating Point Software Package
