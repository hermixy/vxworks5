...
DESCRIPTION
	...
	The entry point __l_sscale is called from __l_do_func to emulate
...
src_small:
	...
	orl		#__l_unfl_mask,a6@(USER_FPSR) | set UNFL
den_done:
	lea		a6@(FPTEMP),a0
	jra 		__l_t_resdnrm
no_denorm:
...
src_pos:
	...
	jeq 		__l_denorm	| if zero, result is denorm
...
den_pos:
	...
	movel		a6@(FPTEMP_HI),a6@(ETEMP_HI)
	movel		a6@(FPTEMP_LO),a6@(ETEMP_LO)
	orl		#__l_unfl_bit,a6@(USER_FPSR)	| set unfl
	lea		a6@(ETEMP),a0
	jra 		__l_t_resdnrm
nden_exit:
	movew		d1,a6@(FPTEMP)	| result in FPTEMP
...
no_carry:
	...
	tstb		a6@(L_SCR2)
	jeq 		tst_zero
	orl		#__l_unfl_inx_mask,a6@(USER_FPSR)
...
| __l_t_resdnrm with exceptional operand in ETEMP.
|
sm_dnrm:
...
neg_zero:
	...
	clrl		a6@(FP_SCR1+8)
/*	fmoves	&0x80000000,fp0 */	 .long 0xf23c4400,0x80000000
	rts
pos_zero:
	clrl		a6@(FP_SCR1)	| clear the exceptional operand
	clrl		a6@(FP_SCR1+4)	| for __x_gen_except.
